			...
			if (scope.$last === true) { // accordion section finished loading - init jQuery plugins after 3 seconds to allow menu items to render first
				$timeout(function(){
					...
					if ($(".Monetization").length === 0){
						$('div:contains("IMA SDK").panel').before('<div class="tabSubHeader Monetization">Monetization</div><p class="categoryDesc">Content monetization settings.</p>'); // add Monetization separator
					}
					if ($(".pwa").length === 0){
