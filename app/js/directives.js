			...
			if (scope.$last === true) { // accordion section finished loading - init jQuery plugins after 3 seconds to allow menu items to render first
				$timeout(function(){
					...
					if ($(".advancedDisclaimer").length === 0){
						$('.jsonView').after('<br><br><br><br><p class="categoryDesc advancedDisclaimer">Note: After editing the above configuration, please click \'Save changes\'. In order to see changes propagate to other studio tabs, please also refresh the page</p>'); // add Advanced tab disclaimer
					}
