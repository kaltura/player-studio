                ...
                function checkItemSections(item, elm) {
                    if (item.sections) {
                        if (item.sections.type == 'tabs') {
                            ...
                            angular.forEach(item.sections.sectionsConfig, function(section, sectionKey) { // create tabs
                                    section.template= [];
                                    var sectionPart = angular.element('<li ka-dynamic-section model="' + item.model + '" section="' + sectionKey + '"></li>');
                                    $(elm).find('li>div[section=' + sectionKey + ']').each(function(index, child) { // add children
                                        section.template.push(child);
                                        $(child).parents('li').remove();
                                    });
