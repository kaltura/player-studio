    ...
    .directive('colorpicker', ['$document', '$compile', 'Color', 'helper', '$timeout', '$rootScope', function($document, $compile, Color, helper, $timeout,$rootScope) {
        return {
            ...
            link: function($scope, elem, attrs, ngModel) {
                ...
                pickerColorBase = colorpickerTemplate.find('.clickarea div')[0].style;
                pickerColorPreview = colorpickerTemplate.find('.clickarea div')[4].style;
                ...
                var update = function(forceVal) {
                    ...
                    previewColor();
                    pickerColorInput.val(pickerColor[thisFormat]());
                };
                ...
                pickerColorInput.bind('keyup', function(event) {
                    var textInput = pickerColorInput.val()
                    var inputColor = pickerColor.setColor(textInput);
                    if (updateRequest) {
                        $timeout.cancel(updateRequest);
                    }
                    updateRequest = $timeout(function() {
                ...
                colorpickerTemplate.find('.clickarea div').bind('click', function(event) {
                    slidersUpdate(event);
                    mousemove(event);
                });

                colorpickerTemplate.find('.clickarea div').bind('mousedown', function(event) {
                    slidersUpdate(event);
